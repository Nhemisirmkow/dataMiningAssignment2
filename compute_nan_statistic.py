...
if __name__ == "__main__":
    ...
    data_path = "./datasets_preprocessed/training_important_rows.csv"
    ...
    df = pd.DataFrame(columns=["column_name", "nans_percentage"])

    for col in data.columns:
        row = {"column_name" : col, "nans_percentage": round(data[col].isna().sum() / number_of_rows * 100, 2)}
        df = df.append(row, ignore_index=True)
        print(row)

    df.to_csv("./datasets_preprocessed/training_important_rows_nans_statistic.csv", index = False)
